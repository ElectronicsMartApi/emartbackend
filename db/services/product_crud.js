...
module.exports = {
    ...
    async view_45_products(skip,limit,rating){
        try{
            ...
            var total_products = await ProductModel.find().count();
            if(products.length!=0){
    ...
    async direct_change(){  //will add or update discount randomly
        try{
            console.log("reached");
            var err = 0;
            var products = await ProductModel.find();
            for(var i = 0 ; i < products.length ; i++){
                var discount = Math.floor(Math.random()*20);
                var update = await ProductModel.updateOne(
                    {
