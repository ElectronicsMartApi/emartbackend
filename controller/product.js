...
const productController = {
    ...
    async view_all_products(request,response){
        ...
        // }
        // else {rating = 0;}
        // var promise = productOperations.view_all_products(rating);
    ...
    async view_45_products(request,response){
        try{
            ...
            var products = await productOperations.view_45_products(parseInt(skip),parseInt(limit),rating);
            if(products){
                response.status(SUCCESS).json({message:messageBundle["product.found"],Products:products.products,Total_products:products.total_products});
            }
            else{
                response.status(NOT_FOUND).json({message:messageBundle["product.notfound"]});
            }
    ...
    async direct_change(request,response){  //this was created for adding random discounts to all products
        try{
            var update = await productOperations.direct_change();
            if(update){
                response.status(SUCCESS).json({message:messageBundle["update.successful"]});
            }
            else{
